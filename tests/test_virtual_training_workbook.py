...
class VirtualTrainingWorkbookTests(unittest.TestCase):
    def test_training_workbook_is_parseable_and_scores_expected_sessions(self) -> None:
        ...
        self.assertEqual([55, 55, 30], [item.total for item in scores["高腾飞"].sessions])
        self.assertEqual(46.7, scores["高腾飞"].process_average)
        self.assertEqual([0, 0, 30], [item.total for item in scores["高大宇"].sessions])
        self.assertEqual(10.0, scores["高大宇"].process_average)
