...
class WebAssetTests(unittest.TestCase):
    ...
    def test_quality_gate_has_locations_and_warning_acknowledgement(self) -> None:
        ...
        self.assertIn('errors === 0 && (warnings === 0 || state.warningsAcknowledged)', APP_JS)
        self.assertNotIn('id="tdr3-scope-confirmation"', INDEX_HTML)
        self.assertNotIn('TDR3纳入范围确认', APP_JS)
        self.assertNotIn('未完成TDR3，不进入下一环节', APP_JS)
        self.assertIn('我已阅读并确认上述检查提醒', INDEX_HTML)
        self.assertIn('可以继续进入评分', APP_JS)
        self.assertNotIn('评审报告可以进入后续评分', APP_JS)
    ...
    def test_quality_check_smoothly_reads_ten_checkpoints_in_three_columns(self) -> None:
        ...
        self.assertIn("grid-template-columns: minmax(0, 1.28fr) minmax(145px, .55fr) minmax(300px, 1.1fr)", STYLES_CSS)
    ...
    def test_interface_uses_reviewer_wording_and_preserves_full_report_name(self) -> None:
        ...
        self.assertIn('class="report-progress-name"><strong class="inline-name-expand">${escapeHtml(report.source_name)}</strong>', APP_JS)
        self.assertNotIn('title="${escapeHtml(report.source_name)}"', APP_JS)
        self.assertNotIn('data-full-name="${escapeHtml(report.source_name)}"', APP_JS)
        self.assertIn(".inline-name-expand:hover", STYLES_CSS)
        self.assertNotIn(".report-progress-name::after", STYLES_CSS)
    ...
    def test_dimension_one_shows_extracted_opinion_evidence_and_source(self) -> None:
        ...
        self.assertIn("session.opinion_evidence.zero_reason", APP_JS)
    ...
    def test_topbar_and_results_follow_confirmed_structure(self) -> None:
        ...
        self.assertIn("专业价值贡献", INDEX_HTML)
        self.assertIn("总考核分数", INDEX_HTML)
        self.assertNotIn("主观分数考核", INDEX_HTML)
        self.assertNotIn(">评分结果<", INDEX_HTML)
        ...
        self.assertIn('expert.participation_session_count', APP_JS)
        self.assertIn('expert.problem_session_count', APP_JS)
        self.assertIn('有效参评场次分布', APP_JS)
        self.assertIn('试算结果 ·', APP_JS)
        ...
        self.assertIn('process_average}／48', APP_JS)
        self.assertIn('participation_score}／6', APP_JS)
        self.assertIn('problem_score}／6', APP_JS)
        self.assertIn("全体评审人的客观分数和主观分数均完成后统一生成", APP_JS)
    ...
    def test_dimension_one_supports_both_workflows_and_numbers_table(self) -> None:
        ...
        self.assertIn('class="analysis-title-line"', INDEX_HTML)
        self.assertIn('${reviewerCount}位评审人、${projectCount}个项目、${rows.length}行', APP_JS)
        self.assertNotIn('id="dimension-one-summary"', INDEX_HTML)
        self.assertNotIn('id="dimension-one-exception-only"', INDEX_HTML)
        self.assertNotIn("dimensionOneExceptionOnly", APP_JS)
        self.assertIn("width: 240px", STYLES_CSS)
        self.assertIn("--dimension-table-width:${tableWidth}px", APP_JS)
        self.assertNotIn("min-width: 2740px", STYLES_CSS)
    ...
    def test_dimension_one_reviewer_search_supports_three_fuzzy_modes(self) -> None:
        ...
        self.assertIn("reviewerMatchesSearch(row.expert, search)", APP_JS)

    def test_dimension_one_project_name_expands_inline_without_a_tooltip(self) -> None:
        self.assertGreaterEqual(APP_JS.count('class="inline-name-expand"'), 2)
        self.assertNotIn('project-name-cell" title=', APP_JS)
        self.assertNotIn("project-name-tooltip", APP_JS)
        self.assertNotIn(".project-name-tooltip::after", STYLES_CSS)
        self.assertEqual(STYLES_CSS.count(".inline-name-expand:hover"), 1)
