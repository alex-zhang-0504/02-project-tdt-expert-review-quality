...
function renderScoreStatistics() {
  ...
  sq("#score-statistics-table").innerHTML = `<table class="score-table score-summary-table"><colgroup><col style="width:12%"><col span="3" style="width:9%"><col span="5" style="width:8%"><col style="width:11%"><col style="width:10%"></colgroup><thead><tr><th>评审人</th><th>TDR1／35</th><th>TDR2／35</th><th>TDR3／35</th><th>参与度／5</th><th>过程表现／40</th><th>专业价值／60</th><th>超额意见奖励</th><th>总分／100</th><th>状态</th><th>依据</th></tr></thead><tbody>${data.rows.map((r, index) =>
    `<tr><th scope="row">${escapeHtml(r.expert_name)}<small>${r.projects}个项目 · ${r.sessions}场</small></th>
      ...
      <td>${scoreText(r.objective_total)}</td><td>${scoreText(r.subjective_total)}</td><td>${scoreText(r.opinion_bonus)}</td><td><strong>${scoreText(r.total)}</strong>${r.uncapped_total > 100 ? '<small>已封顶</small>' : ''}</td>
  ...
  sq("#score-statistics-details").innerHTML = data.rows.map((r, index) => `<details class="score-detail" id="score-detail-${index}">
    ...
    <p class="muted">阶段基础分按35分计算，再乘有效权重；不适用阶段不计权重，待处理阶段不会被剔除。</p>
    <div class="score-statistics-table-wrap"><table class="score-table score-stage-table"><colgroup><col style="width:20%"><col span="8" style="width:10%"></colgroup><thead><tr><th>阶段</th><th>出勤／7</th><th>会签／7</th><th>意见基础／14</th><th>对策／7</th><th>阶段／35</th><th>有效权重</th><th>加权贡献</th><th>超额奖励</th></tr></thead><tbody>
      ${Object.entries(r.stages).map(([s, v]) => `<tr><th>${s}${v.applicable ? '' : '（不适用）'}</th>${['attendance','signoff','opinion','solution'].map(k=>`<td>${scoreText(v.components[k])}</td>`).join('')}
        <td>${scoreText(v.score)}</td><td>${v.weight}％</td><td>${scoreText(v.contribution)}</td><td>${scoreText(v.opinion_bonus)}<small>意见${v.opinions}条－实参${v.attended}场</small></td></tr>`).join('')}
    </tbody></table></div>
    ...
    <p class="muted">意见基础分＝min（意见条数÷实参场次，1）×14；每阶段奖励＝max（意见条数－实参场次，0），直接相加、不乘阶段权重。</p>
    <p>过程表现${scoreText(r.objective_total)}＋专业价值${scoreText(r.subjective_total)}＋奖励${scoreText(r.opinion_bonus)}＝${scoreText(r.uncapped_total)}；最终${scoreText(r.total)}分，最高100分。</p>
