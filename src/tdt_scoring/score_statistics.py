...
RULE_VERSION = "scores-v0.7"
COMPONENTS = (("attendance", "出勤", 7), ("signoff", "会签", 7),
              ("opinion", "意见", 14), ("solution", "对策", 7))
...
def stage_score(sessions):
    ...
    attendance = None if facts["unknown"] else attended / expected * 7
    signoff = facts["signed"] / expected * 7
    if facts["unknown"]:
        reasons.append("出勤状态未知")
    opinion = 0.0 if not opinions else None if facts["unknown"] or not attended else min(opinions / attended, 1) * 14
    ...
    solution = None if facts["pending"] else facts["solutions"] / opinions * 7 if opinions else 0.0
    if facts["pending"]:
        reasons.append(f"{facts['pending']}条意见待识别对策")
    result["components"] = dict(zip((c[0] for c in COMPONENTS), (attendance, signoff, opinion, solution)))
...
def build_statistics(analysis, scope_confirmed=False):
    ...
    for expert in analysis.experts:
        ...
        reasons, process, objective, bonus = [], None, None, None
        ...
        if suspected:
            reasons.append(f"含{suspected}条待确认对策，疑似默认计入")
        uncapped = None if objective is None or subjective is None or bonus is None else objective + subjective + bonus
...
def build_statistics_workbook(analysis, scope_confirmed=False):
    ...
    summary.append(["评审人", "项目数", "应参场次", "TDR1（35）", "TDR2（35）", "TDR3（35）",
                    "阶段加权（35）", "有效参评场次", "参与度数量档", "评审参与度（5）",
                    "评审过程表现（40）", "专业价值贡献（60）", "超额意见奖励", "封顶前合计", "总分（100）", "状态与说明"])
    stage_sheet = wb.create_sheet("阶段计分依据")
    stage_sheet.append(["评审人", "阶段", "适用", "应参", "实参", "意见条数", "出勤分", "会签分", "意见基础分", "对策分", "阶段分", "权重％", "加权贡献", "超额意见奖励（不加权）", "说明"])
    ...
    for row in data["rows"]:
        summary.append([row["expert_name"], row["projects"], row["sessions"],
                        *[row["stages"][stage]["score"] for stage in STAGE_WEIGHTS],
                        ...
                        row["objective_total"], row["subjective_total"], row["opinion_bonus"], row["uncapped_total"], row["total"],
        ...
        for stage, result in row["stages"].items():
            stage_sheet.append([row["expert_name"], stage, "适用" if result["applicable"] else "不适用",
                                ...
                                *[result["components"].get(c[0]) for c in COMPONENTS], result["score"],
                                result["weight"], result["contribution"], result["opinion_bonus"], "；".join(result["reasons"])])
    ...
    info.append(["计分", "阶段内7／7／14／7分，TDR1／2／3权重4／2／4，不适用阶段权重归一；过程35＋参与度5＋专业价值60。"])
    info.append(["奖励", "各阶段max（意见条数－实参场次，0）直接相加，不乘阶段权重；两维加奖励后最高100分，缺失数据不按0处理。"])
