...
- 同一项目既支持一册多Sheet，也支持TDR1／TDR2／TDR3分别成册；统一按项目编码＋Sheet内部“评审阶段”字段聚合。文件名和Sheet名称均不参与阶段识别或质检。
...
- 读取模块按最新年度V0.4规范为每份报告执行10个后端检查节点，只有真实检查完成后才推进进度并记录实际耗时；批量界面只显示每份报告的当前检查项、完成百分比和最终状态，不再陈列24张检查卡或播放固定延时动画。
